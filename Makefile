...
.PHONY: catapult_prefill catapult_prefill_attention_stream catapult_prefill_attention_context catapult_prefill_attention_kv_cache catapult_prefill_attention_q_context_output clean
catapult_prefill:
	mkdir -p $(LOG_DIR)
...
clean:
	...
	rm -f Catapult_*.ccs
	rm -f catapult.log
	rm -f $(LOG_FILE) $(LOG_FILE_PREFILL_ATTENTION_STREAM) $(LOG_FILE_PREFILL_ATTENTION_CONTEXT) $(LOG_FILE_PREFILL_ATTENTION_KV_CACHE) $(LOG_FILE_PREFILL_ATTENTION_Q_CONTEXT_OUTPUT)
