...
void rmsnorm_scale_weight_chunk_fp(
    ...
    int base_index,
  catapult_fp_t scaled_weight_chunk[kParallelMacLaneCount]);
void rmsnorm_apply_scale_chunk_fp(
    const catapult_fp_t input[llm_accel::kHiddenSize],
  const catapult_fp_t scaled_weight_chunk[kParallelMacLaneCount],
    int base_index,
  catapult_fp_t output_chunk[kParallelMacLaneCount]);
...
void rmsnorm_scale_weight_fp(
  ...
  for (int chunk_index = 0; chunk_index < kRmsNormChunkCount; ++chunk_index) {
    const int base_index = chunk_index * kParallelMacLaneCount;
    catapult_fp_t scaled_weight_chunk[kParallelMacLaneCount];
    rmsnorm_scale_weight_chunk_fp(weight, inv_rms, base_index, scaled_weight_chunk);

#pragma hls_unroll yes
    for (int lane = 0; lane < kParallelMacLaneCount; ++lane) {
      scaled_weight[base_index + lane] = scaled_weight_chunk[lane];
    }
  }
}
...
#pragma hls_unroll yes
    for (int lane = 0; lane < kParallelMacLaneCount; ++lane) {
      scaled_weight_chunk[lane] = scaled_weight[base_index + lane];
    }

    rmsnorm_apply_scale_chunk_fp(input, scaled_weight_chunk, base_index, output_chunk);

#pragma hls_unroll yes
    for (int lane = 0; lane < kParallelMacLaneCount; ++lane) {
      output[base_index + lane] = output_chunk[lane];
    }
  }
}
...
#pragma hls_unroll yes
  for (int lane = 0; lane < kParallelMacLaneCount; ++lane) {
    scaled_weight_chunk[lane] = fp_mul_op(weight[base_index + lane], inv_rms);
  }
}
...
#pragma hls_unroll yes
  for (int lane = 0; lane < kParallelMacLaneCount; ++lane) {
    ...
    output_chunk[lane] = fp_mul_op(input[index], scaled_weight_chunk[lane]);
  }
}
...
#pragma hls_unroll yes
  for (int head_offset = 0; head_offset < llm_accel::kNumAttentionHeads; ++head_offset) {
    if (head_offset < head_end - head_base) {
      packet->max_score[head_offset] = fp_const(-1.0e30f);
...
void compute_context_head_state_packet(
    const catapult_fp_t* q_token,
    const catapult_fp_t* k_proj,
    const catapult_fp_t* v_proj,
    int seq_len,
...
void prefill_attention_context_query_token_fp(
    const catapult_fp_t* q_token,
    const catapult_fp_t* k_proj,
    const catapult_fp_t* v_proj,
    int seq_len,
...
void project_hidden_token_bias_fp(
    ...
    int in_tile,
    catapult_fp_t output[llm_accel::kHiddenSize]) {
  catapult_fp_t output_buffer[llm_accel::kHiddenSize];
...
#pragma hls_unroll yes
  for (int index = 0; index < llm_accel::kHiddenSize; ++index) {
    output[index] = output_buffer[index];
  }
}
...
void project_rmsnorm_kv_token_bias_fp(
    const catapult_fp_t input_token[llm_accel::kHiddenSize],
    const catapult_fp_t weight[llm_accel::kHiddenSize],
    const catapult_fp_t& inv_rms,
    const llm_accel::packed_w4_t packed_weights[kKvWidth * llm_accel::kHiddenSize / 2],
  ...
  for (int out_base = 0; out_base < kKvWidth; out_base += out_tile) {
    const int out_extent = min_int(out_tile, kKvWidth - out_base);
ATTN_NORM_KV_PROJ_INIT_LOOP:
#pragma hls_pipeline_init_interval 1
    for (int out_offset = 0; out_offset < out_extent; ++out_offset) {
      partial_sum[out_offset] = bias[out_base + out_offset];
    }

    for (int in_base = 0; in_base < llm_accel::kHiddenSize; in_base += in_tile) {
      const int in_extent = min_int(in_tile, llm_accel::kHiddenSize - in_base);
ATTN_NORM_KV_PROJ_LOAD_LOOP:
#pragma hls_pipeline_init_interval 1
      for (int in_offset = 0; in_offset < in_extent; ++in_offset) {
        const int input_index = in_base + in_offset;
        input_tile[in_offset] = fp_mul_op(
            input_token[input_index],
            fp_mul_op(weight[input_index], inv_rms));
      }

ATTN_NORM_KV_PROJ_ACCUM_LOOP:
#pragma hls_pipeline_init_interval 2
      for (int out_offset = 0; out_offset < out_extent; ++out_offset) {
        const int out_index = out_base + out_offset;
        catapult_fp_t accum = partial_sum[out_offset];
...
void qwen_prefill_attention_hidden_proj_tile_array_core(
  ...
  const prefill_catapult_fp_t partial_sum_tile_in[kProjectionTileCapacity],
  prefill_catapult_fp_t partial_sum_tile_out[kProjectionTileCapacity],
    prefill_catapult_fp_t inv_rms,
    int lane_extent,
    int out_extent,
    bool apply_rmsnorm);

void qwen_prefill_attention_hidden_proj_tile_pointer_core(
    const prefill_catapult_fp_t input_tile[kProjectionTileCapacity],
    const prefill_catapult_fp_t input_layernorm_weight_tile[kProjectionTileCapacity],
...
void init_hidden_proj_one_tile_packet(HiddenProjFpTilePacket* packet) {
#pragma hls_unroll no
  for (int index = 0; index < kProjectionTileCapacity; ++index) {
    packet->data[index] = prefill_catapult_fp_t(1.0f);
  }
}

void mask_hidden_proj_input_tile_packet(int lane_extent, HiddenProjFpTilePacket* packet) {
  for (int index = lane_extent; index < kProjectionTileCapacity; ++index) {
    packet->data[index] = prefill_catapult_fp_t(0.0f);
  }
}
...
void load_hidden_proj_partial_tile_from_output(
    const prefill_catapult_fp_t* output,
    int out_base,
    int out_extent,
    HiddenProjPartialTilePacket* packet) {
#pragma hls_unroll no
  for (int index = 0; index < kProjectionTileCapacity; ++index) {
    packet->data[index] = index < out_extent ? output[out_base + index] : hidden_proj_fp_zero();
  }
}
  ...
  for (int out_base = 0; out_base < kHiddenSize; out_base += proj_tile) {
    ...
    for (int in_base = 0; in_base < kHiddenSize; in_base += proj_tile) {
      ...
      if (apply_rmsnorm && input_layernorm_weight != nullptr) {
        ...
        init_hidden_proj_one_tile_packet(&layernorm_tile_packet);
      }
      ...
      qwen_prefill_attention_hidden_proj_tile_array_core(
          ...
          partial_sum_tile_next_packet.data,
          effective_inv_rms,
          in_extent,
          out_extent,
          apply_rmsnorm);
...
#pragma hls_unroll no
      for (int out_offset = 0; out_offset < kProjectionTileCapacity; ++out_offset) {
        partial_sum_tile_packet.data[out_offset] = partial_sum_tile_next_packet.data[out_offset];
      }
    }
...
#pragma hls_pipeline_init_interval 2
  for (int out_offset = 0; out_offset < kProjectionTileCapacity; ++out_offset) {
    partial_sum_tile_out[out_offset] = fp_add_op(
        partial_sum_tile_in[out_offset],
        weighted_chunk_dot_fp(
  ...
  (void)scales_tile;
  (void)partial_sum_tile_in;
  (void)partial_sum_tile_out;
  (void)inv_rms;
  (void)lane_extent;
  (void)out_extent;
  (void)apply_rmsnorm;
    ...
    if (out_offset < out_extent) {
      partial_sum_tile_out[out_offset] = fp_add_op(
          ...
          weighted_chunk_dot_fp(
              ...
              packed_weights,
              scales_tile,
              global_out_base + out_offset,
              global_in_base,
              in_dim,
              lane_extent));
    } else {
      partial_sum_tile_out[out_offset] = partial_sum_tile_in[out_offset];
    }
  }
#else
  (void)input_tile;
...
void project_hidden_output_range_token_tilewise_fp(
    const prefill_catapult_fp_t* input_token,
    const prefill_catapult_fp_t* input_layernorm_weight,
    const packed_w4_t* packed_weights,
    const prefill_catapult_fp_t* bias,
    ...
    bool apply_rmsnorm,
    int output_base,
    int output_extent_total,
    prefill_catapult_fp_t* output_range) {
#ifdef __SYNTHESIS__
  const int proj_tile = min_int(kProjectionTileCapacity, max_int(1, tile_span));

  for (int out_base = output_base; out_base < output_base + output_extent_total; out_base += proj_tile) {
    const int out_extent = min_int(proj_tile, output_base + output_extent_total - out_base);
    HiddenProjScaleTilePacket scale_tile_packet;
    HiddenProjPartialTilePacket partial_sum_tile_packet;
    HiddenProjPartialTilePacket partial_sum_tile_next_packet;
    const prefill_catapult_fp_t effective_inv_rms = apply_rmsnorm ? inv_rms : hidden_proj_fp_one();
    ...
    for (int in_base = 0; in_base < kHiddenSize; in_base += proj_tile) {
      ...
      qwen_prefill_attention_hidden_proj_tile_pointer_core(
          input_tile_packet.data,
          layernorm_tile_packet.data,
          packed_weights,
          scale_tile_packet.data,
...
#pragma hls_unroll no
    ...
    store_hidden_proj_partial_tile_packet(
        partial_sum_tile_packet,
        out_extent,
        output_range,
  ...
  (void)apply_rmsnorm;
  (void)output_base;
  (void)output_extent_total;
  (void)output_range;
#endif
}

void accumulate_hidden_input_range_token_tilewise_fp(
    const prefill_catapult_fp_t* input_range,
    const packed_w4_t* packed_weights,
    const prefill_catapult_fp_t* scales,
    int input_base,
    int input_extent_total,
    int tile_span,
    prefill_catapult_fp_t* output_token) {
#ifdef __SYNTHESIS__
  const int proj_tile = min_int(kProjectionTileCapacity, max_int(1, tile_span));
  ...
  for (int out_base = 0; out_base < kHiddenSize; out_base += proj_tile) {
    ...
    load_hidden_proj_scale_tile_packet(scales, out_base, out_extent, &scale_tile_packet);
    load_hidden_proj_partial_tile_from_output(output_token, out_base, out_extent, &partial_sum_tile_packet);

    for (int input_offset = 0; input_offset < input_extent_total; input_offset += proj_tile) {
      const int lane_extent = min_int(proj_tile, input_extent_total - input_offset);
      HiddenProjFpTilePacket input_tile_packet;
      HiddenProjFpTilePacket layernorm_tile_packet;

      load_hidden_proj_fp_tile_packet(input_range, input_offset, lane_extent, &input_tile_packet);
      init_hidden_proj_one_tile_packet(&layernorm_tile_packet);
...
#ifdef __SYNTHESIS__
void prefill_attention_context_single_head_fp(
    const catapult_fp_t* q_head,
    const catapult_fp_t* k_proj,
    const catapult_fp_t* v_proj,
    int seq_len,
    int query_index,
    int key_tile,
    int head,
    catapult_fp_t* context_head) {
  const int kv_head = head / kNumGroups;
  const catapult_fp_t attention_scaling = fp_const(0.08838834764831845f);
  catapult_fp_t max_score = fp_const(-1.0e30f);
  catapult_fp_t denom = fp_zero();

  zero_head_accum_128_fp(context_head);

  for (int key_begin = 0; key_begin <= query_index && key_begin < seq_len; key_begin += key_tile) {
    const int query_limit = query_index + 1;
    const int key_end = min_int(seq_len, min_int(query_limit, key_begin + key_tile));
    for (int key_index = key_begin; key_index < key_end; ++key_index) {
      const catapult_fp_t* k_head = k_proj + key_index * kKvWidth + kv_head * kHeadDim;
      const catapult_fp_t score = fp_mul_op(dot_product_128_fp(q_head, k_head), attention_scaling);
      if (fp_gt_op(score, max_score)) {
        max_score = score;
      }
    }
  }
  ...
  for (int key_begin = 0; key_begin <= query_index && key_begin < seq_len; key_begin += key_tile) {
    ...
    for (int key_index = key_begin; key_index < key_end; ++key_index) {
      const catapult_fp_t* k_head = k_proj + key_index * kKvWidth + kv_head * kHeadDim;
      const catapult_fp_t* v_head = v_proj + key_index * kKvWidth + kv_head * kHeadDim;
      const catapult_fp_t score = fp_mul_op(dot_product_128_fp(q_head, k_head), attention_scaling);
      const catapult_fp_t exp_score = approx_exp_fp(fp_sub_op(score, max_score));
      denom = fp_add_op(denom, exp_score);
      scaled_accum_128_fp(exp_score, v_head, context_head);
    }
  }

  const catapult_fp_t inv_denom = fp_gt_op(denom, fp_zero())
      ? approx_reciprocal_fp(denom)
      : fp_zero();
  scale_store_128_fp(context_head, inv_denom, context_head);
}
...
void qwen_prefill_attention_q_context_output_tile_stream_catapult(
  ...
  if (apply_rmsnorm) {
    ...
    init_hidden_proj_one_tile_packet(&input_layernorm_weight_tile_packet);
  }
  ...
  read_hidden_proj_fp_tile_words(partial_sum_tile_in_chan, partial_sum_tile_in_packet);
  ...
  qwen_prefill_attention_hidden_proj_tile_array_core(
      ...
      packed_weight_tile_packet.data,
      scale_tile_packet.data,
        partial_sum_tile_in_packet.data,
        partial_sum_tile_out_packet.data,
      effective_inv_rms,
      lane_extent,
      out_extent,
      apply_rmsnorm);

      write_hidden_proj_fp_tile_words(partial_sum_tile_out_packet, partial_sum_tile_out_chan);
}
...
ATTN_KV_CACHE_INIT_LOOP:
#pragma hls_unroll no
  for (int index = 0; index < kPrefillSeqCapacity * kKvWidth; ++index) {
    k_cache[index] = fp_zero();
    v_cache[index] = fp_zero();
  }
...
#pragma hls_pipeline_init_interval 2
    for (int token_index = token_begin; token_index < token_end; ++token_index) {
      const catapult_fp_t* input_token = input_sequence + token_index * llm_accel::kHiddenSize;
      const catapult_fp_t mean_square = fp_mul_op(rmsnorm_square_sum_fp(input_token), fp_const(1.0f / 1536.0f));
      const catapult_fp_t inv_rms = approx_rsqrt_fp(fp_add_op(mean_square, rms_eps));
      catapult_fp_t* k_proj_token = k_cache + token_index * kKvWidth;
      catapult_fp_t* v_proj_token = v_cache + token_index * kKvWidth;

      project_rmsnorm_kv_token_bias_fp(
        input_token,
        input_layernorm_weight,
        inv_rms,
          k_packed_weights,
...
INIT_Q_CONTEXT_OUTPUT_BUFFER:
#pragma hls_unroll no
  for (int token_index = 0; token_index < kPrefillSeqCapacity; ++token_index) {
...
KernelStatus qwen_prefill_attention_kernel_catapult(
    ...
    catapult_fp_t v_cache[kPrefillSeqCapacity * kKvWidth],
    catapult_fp_t output_sequence[kPrefillSeqCapacity * kHiddenSize]) {
  KernelStatus status = {true, 0};
  if (seq_len <= 0 || tile_config.seq <= 0 || tile_config.query <= 0 || tile_config.key <= 0 || tile_config.hidden_proj <= 0 ||
      tile_config.kv_proj <= 0 || tile_config.head_dim != kHeadDim || tile_config.query_heads_parallel <= 0 ||
      tile_config.kv_heads_parallel <= 0) {
    status = {false, 1};
  } else if (seq_len > kPrefillSeqCapacity || tile_config.seq > kPrefillSeqCapacity ||
             tile_config.query > kPrefillQueryCapacity || tile_config.key > kPrefillKeyCapacity ||
             tile_config.hidden_proj > kProjectionTileCapacity || tile_config.kv_proj > kProjectionTileCapacity ||
             tile_config.query_heads_parallel > kNumAttentionHeads || tile_config.kv_heads_parallel > kNumKeyValueHeads) {
    status = {false, 2};
  ...
  PrefillAttentionTileConfig effective_tile_config = tile_config;
  int effective_seq_len = seq_len;
  if (!status.ok) {
    effective_seq_len = 0;
    effective_tile_config.seq = 1;
    effective_tile_config.query = 1;
    effective_tile_config.key = 1;
    effective_tile_config.hidden_proj = 1;
    effective_tile_config.kv_proj = 1;
    effective_tile_config.head_dim = kHeadDim;
    effective_tile_config.query_heads_parallel = 1;
    effective_tile_config.kv_heads_parallel = 1;
  } else {
    qwen_prefill_attention_kv_cache_stage_catapult(
        input_sequence,
        effective_seq_len,
        effective_tile_config,
        input_layernorm_weight,
        rms_eps,
        k_packed_weights,
        v_packed_weights,
...
KernelStatus qwen_prefill_attention_kernel_catapult(
    ...
    CatapultTensorView<prefill_catapult_fp_t> v_cache,
    CatapultTensorView<prefill_catapult_fp_t> output_sequence) {
  return qwen_prefill_attention_kernel_catapult(
      input_sequence.data,
      seq_len,
      tile_config,
      input_layernorm_weight.data,
