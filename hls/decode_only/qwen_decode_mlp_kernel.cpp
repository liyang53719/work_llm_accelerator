...
KernelStatus qwen_decode_mlp_kernel(
    const scalar_t* attention_residual_token,
  ...
  if (attention_residual_token == nullptr || output_token == nullptr || post_attention_layernorm_weight == nullptr ||
      gate_packed_weights == nullptr || up_packed_weights == nullptr || down_packed_weights == nullptr ||
      gate_scales == nullptr || up_scales == nullptr || down_scales == nullptr) {
    return {false, 1};
  }
  ...
  rmsnorm_token(attention_residual_token, post_attention_layernorm_weight, rms_eps, post_norm.data());
  ...
  for (int index = 0; index < kHiddenSize; ++index) {
    output_token[index] = attention_residual_token[index] + down_proj[index];
  }
