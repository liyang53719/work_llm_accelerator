...
constexpr int kInvalidDescriptorError = 10;
constexpr int kInvalidPortError = 11;
...
KernelStatus qwen_decode_top_wrapper(
  ...
  if (!valid_layer_id(descriptor.layer_id) || descriptor.past_seq_len < 0) {
  ...
  const scalar_t* input_token = scalar_ptr(ports.activation_ddr, descriptor.input_token_addr);
  ...
  const scalar_t* o_scales = scalar_ptr(ports.scale_ddr, descriptor.layer_scales_base_addr + layout.o_scale_offset_bytes);
    ...
    std::vector<scalar_t> attention_output(kHiddenSize, 0.0f);
...
DispatchStatus qwen_decode_dispatch_layers(
  ...
  for (int index = 0; index < layer_count; ++index) {
    KernelStatus status = qwen_decode_top_wrapper(descriptors[index], ports);
    if (!status.ok) {
